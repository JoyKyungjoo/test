...
class Test3 extends Component {
    ...
    handleRemove = (id) => {
        // 클릭하는 영역의 번호 console로 노출 확인용
        // console.log(id);
        
        this.setState({
            movies : this.state.movies.filter(item => item.no !== id)
        })
    }
    render() {
        return (
            <div className="gallery">
                ...
                {/* <article>
                    <div className="left">
                        <img src={this.state.movies[0].poster} alt="영화 포스터" />
                    </div>
                    <div className="right">
                        <h3>{this.state.movies[0].title}</h3>
                        <ul>
                            <li>출연진 : {this.state.movies[0].actor}</li>
                            <li>감독 : {this.state.movies[0].director}</li>
                            <li>개봉일 : {this.state.movies[0].date}</li>
                        </ul>
                        <button>제거</button>
                    </div>
                </article> */}
                ...
                {/* map 사용하여 노출 */}
                {
                    this.state.movies.map(item =>
                        // map사용시, 아래와 같이 key값이 필요함!!!
                        // Q. key값은 언제 필요한거지...? map 사용하면, 무조건?
                        <article key={item.no}>
                            <div className="left">
                            ...
                            <div className="right">
                                ...
                                <button onClick={() => this.handleRemove(item.no)}>제거</button>
                            </div>
                        </article>
                    )
                }
